...
.setFooter(`Requested by ${track.requestedBy.username} | Made By Whirl#0021`)
     ...
     await interaction.editReply({embeds: [playEmbed], ephemeral: true})  }
        if(interaction.commandName == "stop") {
            if (!interaction.member.voice.channel) return interaction.channel.send(`:x: | - You're not in a voice channel !`);

            if (interaction.guild.me.voice.channel && interaction.member.voice.channel.id !== interaction.guild.me.voice.channel.id) return interaction.channel.send(`:x: | - You are not in the same voice channel !`);
    
            const queue = player.getQueue(interaction.guild.id);
    
       if(!queue) return interaction.reply(`:x: | - There is no music playing  in this guild !`);
    ...
    let volume = interaction.options.getInteger("volume");
            if (queue) {
                ...
                return interaction.reply(`:white_check_mark: | - Volume set to ${volume} !`);
            };
        }
       ...
       if(!queue) return interaction.reply(`:x: | - There is no music playing  in this guild !`);
            ...
            if (queue) {
                ...
                .setColor(`RANDOM`)
                .setTitle(`🎶 | Queue`)
                .setDescription(`${queue.tracks.map((song, index) => `**${index + 1}** - **${song.title}**`).join("\n")}`)
                .setFooter(`Now Playing: ${queue.nowPlaying.track} | Made By Whirl#0021`)
